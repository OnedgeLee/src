        ...
        while True :
            ...
            if len(centroids) == 2 :
                ...
                if not VEC_INIT :
                    ...
                    da = pd.DataFrame(data=np.array([vc_a1_]), columns=['x', 'y', 'z'])
                    db = pd.DataFrame(data=np.array([vc_b1_]), columns=['x', 'y', 'z'])    
                    dc = pd.DataFrame(data=np.array([vc_c1_]), columns=['x', 'y', 'z'])
                    df = df.append(da)
                    df = df.append(db)
                    df = df.append(dc)
                    ...
                    VEC_INIT = True
                parsed = raw_string_parsing(last_received)
                if len(parsed) > 2 :
                    count += 1
                    r = float(parsed[0])
                    p = float(parsed[1])
                    ...
                    # Make DataFrame using Pandas
                    do = pd.DataFrame(data=np.array([vc_o2]), columns=['x', 'y', 'z'])
                    da = pd.DataFrame(data=np.array([vc_a2_]), columns=['x', 'y', 'z'])
                    db = pd.DataFrame(data=np.array([vc_b2_]), columns=['x', 'y', 'z'])    
                    dc = pd.DataFrame(data=np.array([vc_c2_]), columns=['x', 'y', 'z'])
                    df = df.append(do)
                    df = df.append(da)
                    df = df.append(db)
                    df = df.append(dc)
