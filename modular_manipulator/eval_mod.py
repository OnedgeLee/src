...
p      = rs.pipeline()
rs_cfg = rs.config()
rs_cfg.enable_device(detected)
rs_cfg.enable_stream(rs.stream.depth, 640, 480, rs.format.z16, rs_fps)
rs_cfg.enable_stream(rs.stream.color, 1280, 720, rs.format.bgr8, rs_fps)
...
# def prep_display(dets_out, img, d_img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):    
    """
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        frs  = p.wait_for_frames()
        a_fs = align.process(frs)
        ...
        mask = inv_alph_masks.byte().cpu().numpy()
        d_cmap, = map(torch.tensor, (d_cmap,))
        d_cmap  = d_cmap * inv_alph_masks.prod(dim=0) + masks_color_summand
        """
            Code HERE!
        """
    ...
    # img_numpy = (d_cmap * 255).byte().cpu().numpy()
    try :
        img_numpy = d_cmap.byte().cpu().numpy()
        img_numpy[:, :] = np.where(mask==0, img_numpy[:, :], 0)
        padded_mask = np.zeros((img_numpy.shape[0]+2, img_numpy.shape[1]+2), dtype=np.uint8)
        thresh = cv2.threshold(padded_mask, 120, 255, cv2.THRESH_BINARY)[1]
        cnts = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        cnts = cnts[0] if len(cnts) == 2 else cnts[1]
        for c in cnts :
            cv2.drawContours(img_numpy, [c], -1, (255, 0, 0) -1)
    except :
        img_numpy = (img_gpu * 255).byte().cpu().numpy()
    ...
    if args.display_fps:
        ...
        # cv2.putText(d_cmap, fps_str, text_pt, font_face, font_scale, text_color, font_thickness, cv2.LINE_AA)
...
def evalvideo(net:Yolact, path:str, out_path:str=None):
    ...
    def get_next_frame(_p):
        frames = []
        for idx in range(args.video_multiframe):
            frs   = _p.wait_for_frames()
            a_fs  = align.process(frs)
            c_f   = a_fs.get_color_frame()
            frame = cv2.resize(np.asanyarray(c_f.get_data()), dsize=(frame_width, frame_height), interpolation=cv2.INTER_AREA)
            # c_img = cv2.resize(c_img, dsize=(frame_width, frame_height), interpolation=cv2.INTER_AREA)
            # frame = c_img
            # d_f  = a_fs.get_depth_frame()
            # d_img = np.asanyarray(d_f.get_data())
            # d_img = cv2.resize(d_img, dsize=(frame_width, frame_height), interpolation=cv2.INTER_AREA)
            if frame is None:
                return frames
            frames.append(frame)
            # frames.append(d_img)
    ...
    def transform_frame(frames):
        with torch.no_grad():
            # d_img  = frames[-1]
            # tmp    = frames[0:args.video_multiframe]
            # frames = [torch.from_numpy(frame).cuda().float() for frame in frames[0:args.video_multiframe]]
            frames = [torch.from_numpy(frame).cuda().float() for frame in frames]
            # print("transfrom length", len(frames))
            # return frames, transform(torch.stack(frames[0:args.video_multiframe], 0)), d_img
            return frames, transform(torch.stack(frames, 0))

    def eval_network(inp):
        with torch.no_grad():
            # frames, imgs, d_img = inp
            frames, imgs = inp
            num_extra = 0
    ...
    def prep_frame(inp, fps_str):
        ...
        """
        with torch.no_grad():
            # frame, preds, d_img = inp
            frame, preds = inp
            # return prep_display(preds, frame, d_img,  None, None, undo_transform=False, class_color=True, fps_str=fps_str)
            ...
            while running:
                ...
                if not frame_buffer.empty():
                    ...
                    if out_path is None:
                        ...
                        # img2 = frame_buffer.get()
                        cv2.imshow(path, img1)
                        # d_cmap = cv2.applyColorMap(cv2.convertScaleAbs(img2, alpha=0.03), cv2.COLORMAP_JET)
                        # cv2.imshow("depth", d_cmap)
                    else:
    ...
    first_batch = eval_network(transform_frame(get_next_frame(p)))
    print('Done.')
    ...
    try:
        while running:
            ...
            if not vid_done:
                next_frames = pool.apply_async(get_next_frame, args=(p, ))
            else:
                next_frames = None
            ...
            if not (vid_done and len(active_frames) == 0):
                ...
                for frame in active_frames:
                    ...
                    if frame['idx'] == 0:
                        ...
                        # print(frame['value'][1][0]['detection']['mask'])
                ...
                for frame in active_frames:
                    if frame['idx'] == 0:
                        # print(frame)
                        fr = frame['value'].get()
                        # print('fr length', len(fr))
                        # print("mask", frame['mask'].get())
                        # frame_buffer.put(frame['value'].get())
                        frame_buffer.put(fr)
                # frame_buffer.put(d_img)
